...
const LeftSidebar = () => {
  ...
  const handleNotificationView = () => {
    if (notificationsData?.documents && !hasMarkedRead.current) {
      const unread = notificationsData.documents.filter((n: any) => !n.isRead);
      if (unread.length > 0) {
        unread.forEach((n: any) => markAsRead(n.$id));
  ...
  useEffect(() => {
    if (notificationsData?.documents) {
      const unread = notificationsData.documents.some((n: any) => !n.isRead);
